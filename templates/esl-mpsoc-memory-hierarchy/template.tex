...
\begin{tikzpicture}[
    ...
    tilebox/.style={draw=core-accent!70!black, rounded corners=3pt},
    ...
    % esl-architecture-block/esl-mpsoc-tile-array): a fit box needs MORE
    % headroom above its contents than on its other sides, so the corner
    % label (drawn outside the fit box, not inside it) never overlaps the
    % topmost contained node -- the label-vs-PE collision this replaces.
    ...
    bottom/.initial=6pt, top/.initial=16pt,
  ]
  ...
  \foreach \t in {1,...,\ntiles} {
    ...
    \begin{scope}[on background layer]
      \node[tilebox, fit margins={}, fit=\tilefitlist] (tile-\t) {};
    \end{scope}
  ...
  % tile was this template's first draft. \rail sits 10pt above the tile
  % tops (rowmid) -- clear of the tiles' own border -- and well below mem,
  % so the riser/rail/stem segments never coincide with either (an earlier
  % attempt placed the rail AT rowmid's own height, which is the tile
  % boxes' own border height -- it ran directly on top of them).
  \coordinate (rail) at ($(rowmid)+(0,10pt)$);
  \foreach \t in {1,...,\ntiles} {
    \draw[interbus] (tile-\t.north) -- (tile-\t.north |- rail);
  }
  \draw[interbus] (tile-1.north |- rail) -- (tile-\ntiles.north |- rail);
  \draw[interbus] (rail) -- (mem);
