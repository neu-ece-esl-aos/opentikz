...
\begin{tikzpicture}[
  ...
  % name=<id> (cp-4883/WP-11 family-wide ruling, circuitikz-bipole-node-identity) --
  % gives the bipole itself a PGF node identity, so it is visible to
  % tools/render_selfcheck.py's detect_node_node_collisions.
  \draw (kcl-1) to[R=$R_{\text{sense}}$, o-o, name=rsense] (dev-1);
  \draw (kcl-2) to[I, l=$I_{\text{ref}}$, o-o, name=iref] (dev-2);
