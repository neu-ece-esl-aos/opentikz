...
\begin{tikzpicture}[
  ...
  \node[terminal node, above=26pt of tilemid] (io) {\iolabel};
  ...
  % intra/inter-bus at ports") -- a straight tile.north -- io diagonal was
  % the pre-existing cp-4856-port defect this replaces. \trunk sits 10pt
  % above the tile tops -- clear of the tiles' own dashed border -- and 16pt
  % below io, so the riser/trunk/stem segments never coincide with either.
  % (An earlier fix attempt placed the trunk AT tile1.north's own height,
  % which is the tile box's own border -- the trunk ran directly on top of
  % it. \trunk's explicit y-offset is what avoids that.)
  \coordinate (trunk) at ($(tilemid)+(0,10pt)$);
  \draw[interbus] (tile1.north) -- (tile1.north |- trunk)
    -- (tile2.north |- trunk) -- (tile2.north);
  \draw[interbus] (trunk) -- (io);
